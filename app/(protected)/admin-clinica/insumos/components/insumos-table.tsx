...
export function InsumosTable({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((insumo) => {
      ...
      const descricao = insumo.descricao?.toLowerCase() || "";
      const unidade = insumo.unidade?.toLowerCase() || "";
      const status = insumo.ativo ? "ativo" : "inativo";
      return (
  ...
  const columns: ColumnDef<Insumo>[] = React.useMemo(
    () => [
      {
    ...
    <div className="flex flex-col gap-4 overflow-auto">
      <div className="overflow-hidden px-6 pt-6">
        ...
        <div className="text-muted-foreground hidden flex-1 text-xs lg:flex">
          {table.getFilteredRowModel().rows.length} insumo(s) encontrado(s).
        </div>
