...
export function PacienteDialog({
  ...
  const onSubmit = async (data: PacienteFormValues) => {
    try {
      ...
      const payload: any = {
        nome: data.nome,
        cpf: removeMask(data.cpf), // Remove máscara do CPF antes de enviar
        dataNascimento: data.dataNascimento,
        sexo: data.sexo,
      };

      // Adicionar apenas campos que têm valor (removendo máscaras)
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          ...
          // Retorna sem lançar erro para não fechar o diálogo
          return;
        }
      ...
      // Não fecha o diálogo em caso de erro para permitir correção
    } finally {
      setLoading(false);
    }
  };
                    ...
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                      ...
                      <div className="space-y-1 leading-none">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Desmarque para desativar o paciente no sistema
                        </p>
                      </div>
                    </FormItem>
                  )}
