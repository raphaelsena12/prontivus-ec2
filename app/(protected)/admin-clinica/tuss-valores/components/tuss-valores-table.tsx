...
interface TussValoresTableProps {
  data: TussValor[];
  onDelete?: (valor: TussValor) => void;
  newButtonUrl?: string;
}
...
export function TussValoresTable({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((valor) => {
      const codigoTuss = valor.codigoTuss.codigoTuss?.toLowerCase() || "";
      const descricao = valor.codigoTuss.descricao?.toLowerCase() || "";
      const operadora = valor.operadora?.razaoSocial?.toLowerCase() || "";
      const plano = valor.planoSaude?.nome?.toLowerCase() || "";
      return codigoTuss.includes(search) || descricao.includes(search) || operadora.includes(search) || plano.includes(search);
    });
  }, [data, globalFilter]);

  const columns: ColumnDef<TussValor>[] = React.useMemo(
    () => [
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div>{row.original.tipoConsulta?.nome || "-"}</div>
        ),
      },
      {
        accessorKey: "valor",
        ...
        cell: ({ row }) => (
          <div className="flex justify-end gap-2">
            <Button
              ...
              onClick={() =>
                router.push(
                  `/admin-clinica/tuss-valores/editar/${row.original.id}`
                )
              }
              title="Editar valor TUSS"
    ...
    <div className="flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 lg:px-6 pt-4">
        <div className="text-muted-foreground hidden flex-1 text-xs lg:flex">
          {table.getFilteredRowModel().rows.length} valor(es) TUSS encontrado(s).
        </div>
