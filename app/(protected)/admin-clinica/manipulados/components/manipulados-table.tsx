...
export function ManipuladosTable({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((manipulado) => {
      ...
      const medicoNome = manipulado.medico?.usuario?.nome?.toLowerCase() || "";
      return descricao.includes(search) || informacoes.includes(search) || medicoNome.includes(search);
    });
  }, [data, globalFilter]);
  ...
  const columns: ColumnDef<Manipulado>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Médico",
        cell: ({ row }) => (
          <div className="font-medium">{row.original.medico?.usuario?.nome || "-"}</div>
        ),
      },
      {
      ...
      <div className="flex items-center justify-end">
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input 
              type="search"
              placeholder="Buscar por descrição, informações ou médico..." 
              value={globalFilter}
