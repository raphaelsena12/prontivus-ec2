...
export function OperadorasContent({ clinicaId }: OperadorasContentProps) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const response = await fetch(`/api/admin-clinica/operadoras/${deleteTarget.id}`, { method: "DELETE" });
      if (!response.ok) {
        const data = await response.json();
                ...
                {loading ? (
                  ...
                  operadoras.map((operadora) => (
                    <TableRow key={operadora.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-[10px] py-0.5 px-1.5 leading-tight">{operadora.planosSaude.length} plano(s)</Badge>
                      </TableCell>
                      <TableCell>
                        {operadora.ativo ? (
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => setDeleteTarget(operadora)}
    ...
    <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Tem certeza que deseja excluir a operadora <strong>{deleteTarget?.razaoSocial}</strong>? Esta ação não pode ser desfeita.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
