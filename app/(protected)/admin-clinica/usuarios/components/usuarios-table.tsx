...
const getTipoLabel = (tipo: TipoUsuario) => {
  const labels: Record<TipoUsuario, string> = {
    [TipoUsuario.ADMIN_CLINICA]: "Admin Clínica",
    [TipoUsuario.MEDICO]: "Médico",
    [TipoUsuario.SECRETARIA]: "Secretária",
    [TipoUsuario.PACIENTE]: "Paciente",
    [TipoUsuario.SUPER_ADMIN]: "Super Admin",
  };
...
export function UsuariosTable({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((usuario) => {
      ...
      return nome.includes(search) || email.includes(search) || cpf.includes(search);
    });
  }, [data, globalFilter]);
  ...
  const columns: ColumnDef<Usuario>[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Email",
        cell: ({ row }) => (
          <div className="max-w-md truncate">{row.original.email}</div>
        ),
      },
      {
        accessorKey: "cpf",
