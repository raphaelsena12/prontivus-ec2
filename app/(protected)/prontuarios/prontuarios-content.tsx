...
interface ProntuariosContentProps {
  clinicaId: string | null;
}

export function ProntuariosContent({ clinicaId }: ProntuariosContentProps) {
  ...
  const loadProntuarios = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin-clinica/prontuarios?${params.toString()}`
      );
      if (!response.ok) {
      ...
      <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
        ...
        <div className="px-4 lg:px-6">
          <div className="mb-4">
            ...
            <Input
              id="search"
              placeholder="Buscar por paciente, CPF, médico ou diagnóstico..."
              value={search}
