  ...
  try {
    ...
    for (let i = 0; i < normalizedRows.length; i++) {
      ...
      try {
        ...
        const tipo = (row.tipo || row.tipo_pagamento || "DINHEIRO")
          .toString()
          .toUpperCase()
          .replace(/\s/g, "_");
        ...
        if (!tiposValidos.includes(tipo)) {
          erros.push(
            `Linha ${linha}: Tipo inválido. Use: ${tiposValidos.join(", ")}`
          );
          ignorados++;
          continue;
        }
    ...
    console.error("Erro ao processar upload de formas de pagamento:", error);
    return NextResponse.json(
