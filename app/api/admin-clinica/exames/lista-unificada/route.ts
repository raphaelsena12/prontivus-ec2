...
export async function GET(request: NextRequest) {
  try {
    ...
    const [exames, codigosTuss] = await Promise.all([
    ...
    const tipoExameClinica: Record<string, string> = {
      LABORATORIAL: "Laboratorial",
      IMAGEM: "Imagem",
      OUTROS: "Outros",
    };

    const clinicItems = exames.map((e) => {
      const descricao =
        [e.nome, e.descricao].filter(Boolean).join(" — ") || e.nome;
      return {
        rowId: `clinica:${e.id}`,
        origem: "CLINICA",
        sourceId: e.id,
        codigo: e.codigoTuss?.codigoTuss ?? "—",
        descricao,
        tipo: e.codigoTuss?.sipGrupo ?? null,
        categoria:
          e.codigoTuss?.categoriaProntivus ??
          (e.tipo ? tipoExameClinica[e.tipo] ?? e.tipo : null),
        ativo: e.ativo,
      };
    });
    ...
    console.error("Erro em GET exames/lista-unificada:", error);
    return NextResponse.json(
